//#region node_modules/@rerun-io/web-viewer/index.js
var get_wasm_bindgen = null;
var _wasm_module = null;
async function fetch_viewer_js(base_url) {
	return (await import("./re_viewer-CvfFPvyF.js")).default;
}
async function fetch_viewer_wasm(base_url, on_progress) {
	//!<INLINE-MARKER-OPEN>
	const url = base_url ? new URL("./re_viewer_bg.wasm", base_url) : new URL("../../@rerun-io/web-viewer/re_viewer_bg.wasm", import.meta.url);
	const response = await fetch(url);
	if (!response.ok) throw new Error(`Failed to fetch viewer WASM: ${response.status} ${response.statusText}`);
...
function wrap_fetch_with_progress(response, on_progress) {
	...
	const tracked = new ReadableStream({ async start(controller) {
...
async function load(base_url, on_progress) {
	if (!get_wasm_bindgen || !_wasm_module) [get_wasm_bindgen, _wasm_module] = await Promise.all([fetch_viewer_js(base_url), WebAssembly.compileStreaming(fetch_viewer_wasm(base_url, on_progress))]);
...
var _minimize_current_fullscreen_viewer = null;
function randomId() {
...
var WebViewer = class {
	...
	*/
	async start(rrd, parent, options, open_options = null) {
    ...
    `;
		...
		const on_progress = (received, total) => {
			...
			} else progress_text.textContent = format_mib(received);
		};
		await delay(0);
		let base_url = options?.base_url;
		if (base_url) delete options.base_url;
		let WebHandle_class;
		try {
		...
		if (rrd) this.open(rrd, open_options ?? void 0);
		let self = this;
		function check_for_panic() {
			if (self.#handle?.has_panicked()) self.#fail("Rerun has crashed.", self.#handle?.panic_message());
			else setTimeout(check_for_panic, 1e3);
		}
		check_for_panic();
	}
	#raw_events = /* @__PURE__ */ new Set();
	#dispatch_raw_event(event_json) {
		for (const callback of this.#raw_events) callback(event_json);
	}
	/** Internal interface */
	...
	#dispatch_event(event, ...args) {
		setTimeout(() => {
			...
			if (callbacks) for (const [callback, { once }] of [...callbacks.entries()]) {
				callback(...args);
	...
	on(event, callback) {
		const callbacks = this.#event_map.get(event) ?? /* @__PURE__ */ new Map();
	...
	off(event, callback) {
		...
		else console.warn("Attempted to call `WebViewer.off` with an unregistered callback. Are you passing in the same function instance?");
	}
	/**
	...
	*/
	open(rrd, options = {}) {
		if (!this.#handle) throw new Error(`attempted to open \`${rrd}\` in a stopped viewer`);
	...
	*/
	close(rrd) {
		if (!this.#handle) throw new Error(`attempted to close \`${rrd}\` in a stopped viewer`);
		const urls = Array.isArray(rrd) ? rrd : [rrd];
		for (const url of urls) try {
	...
	*/
	stop() {
		if (this.#state === "stopped") return;
		if (this.#allow_fullscreen && this.#canvas && this.#fullscreen) this.#minimize();
		this.#state = "stopped";
		this.#canvas?.remove();
		this.#clearLoader();
		try {
	...
	#fail(message, error_message) {
      ...
      `;
			...
			if (error_message) errorEl.textContent = error_message;
			else errorEl.remove();
	...
	*/
	open_channel(channel_name = "rerun-io/web-viewer") {
		if (!this.#handle) throw new Error(`attempted to open channel \"${channel_name}\" in a stopped web viewer`);
		...
		const on_send = (data) => {
			if (!this.#handle) throw new Error(`attempted to send data through channel \"${channel_name}\" to a stopped web viewer`);
			try {
	...
	*/
	override_panel_state(panel, state) {
		if (!this.#handle) throw new Error(`attempted to set ${panel} panel to ${state} in a stopped web viewer`);
		try {
	...
	*/
	toggle_panel_overrides(value) {
		if (!this.#handle) throw new Error(`attempted to toggle panel overrides in a stopped web viewer`);
		try {
	...
	*/
	get_active_recording_id() {
		if (!this.#handle) throw new Error(`attempted to get active recording id in a stopped web viewer`);
	...
	*/
	set_active_recording_id(value) {
		if (!this.#handle) throw new Error(`attempted to set active recording id to ${value} in a stopped web viewer`);
	...
	*/
	get_playing(recording_id) {
		if (!this.#handle) throw new Error(`attempted to get play state in a stopped web viewer`);
	...
	*/
	set_playing(recording_id, value) {
		if (!this.#handle) throw new Error(`attempted to set play state to ${value ? "playing" : "paused"} in a stopped web viewer`);
	...
	*/
	get_current_time(recording_id, timeline) {
		if (!this.#handle) throw new Error(`attempted to get current time in a stopped web viewer`);
	...
	*/
	set_current_time(recording_id, timeline, time) {
		if (!this.#handle) throw new Error(`attempted to set current time to ${time} in a stopped web viewer`);
	...
	*/
	get_active_timeline(recording_id) {
		if (!this.#handle) throw new Error(`attempted to get active timeline in a stopped web viewer`);
	...
	*/
	set_active_timeline(recording_id, timeline) {
		if (!this.#handle) throw new Error(`attempted to set active timeline to ${timeline} in a stopped web viewer`);
	...
	*/
	get_time_range(recording_id, timeline) {
		if (!this.#handle) throw new Error(`attempted to get time range in a stopped web viewer`);
	...
	*/
	toggle_fullscreen() {
		...
		if (!this.#handle || !this.#canvas) throw new Error(`attempted to toggle fullscreen mode in a stopped web viewer`);
		if (this.#fullscreen) this.#minimize();
		else this.#maximize();
	}
	set_credentials(access_token, email) {
		if (!this.#handle) throw new Error(`attempted to set credentials in a stopped web viewer`);
...
var css = `
  ...
  .${classes.transition} {
    transition: all ${transition_delay_ms / 1e3}s linear;
  }
...
function setupGlobalEventListeners() {
	window.addEventListener("keyup", (e) => {
		if (e.code === "Escape") _minimize_current_fullscreen_viewer?.();
	});
}
//#endregion
export { LogChannel, WebViewer };

//# sourceMappingURL=@rerun-io_web-viewer.js.map